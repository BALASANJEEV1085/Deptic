...
func (h *ScanHandler) runFixPRBackground(scanID, userID, owner, repo, ghToken, ecosystem string) {
	...
	if !repoData.Permissions.Push {
		fail("SBOM.io does not have write access to this repository.")
		return
	}
	...
	branchName := fmt.Sprintf("sbomio/fix-cves-%s", timestamp)
	...
	if err := createBranch(branchName); err != nil {
		if err.Error() == "branch exists" {
			branchName = fmt.Sprintf("sbomio/fix-cves-%s-%d", timestamp, rand.Intn(9999))
	...
	sb.WriteString("\n> **Warning:** After merging, run your test suite to verify compatibility. Version bumps may include breaking changes.\n\n_Generated by SBOM.io — Versions verified clean against OSV.dev vulnerability database_")

	prBody := map[string]string{
		"title": fmt.Sprintf("fix(security): patch all %d vulnerable packages to clean versions [SBOM.io]", len(updates)),
		"body":  sb.String(),
		"head":  branchName,
		"base":  defaultBranch,
	}
