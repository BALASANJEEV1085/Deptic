...
function PanelMock({ index }: { index: number }) {
  ...
  const Icon = f.icon
  return (
    <div className="flex h-full w-full flex-col justify-center rounded-2xl border border-[#1a1a1a] bg-[#0a0a0a] p-8">
      <Icon className="size-8 text-white" />
      <h3 className="mt-5 font-heading text-2xl font-bold text-white md:text-3xl">
        {f.name}
      </h3>
      <p className="mt-3 max-w-sm text-base text-[#888888]">{f.desc}</p>
      <div className="mt-6 flex flex-wrap gap-2">
        {f.tags.map((t) => (
          <span
            key={t}
            className="rounded-full border border-[#1a1a1a] bg-[#111111] px-3 py-1 font-mono text-xs text-white"
          >
            {t}
          </span>
        ))}
    ...
    <section ref={ref} className="relative bg-black" style={{ height: '400vh' }}>
      <div className="sticky top-0 flex h-screen items-center overflow-hidden px-6 md:px-12">
        <div className="mx-auto grid w-full max-w-7xl grid-cols-1 items-center gap-12 md:grid-cols-[36%_64%]">
          ...
          {/* mobile active label */}
          <p className="text-sm text-[#888888] md:hidden">
