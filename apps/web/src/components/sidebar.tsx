...
export function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="flex h-full w-full flex-col border-r border-border bg-[hsl(var(--sidebar-bg))] text-zinc-400">
      {/* Workspace Selector */}
      <div className="hidden md:flex h-12 items-center px-3 mb-1">
        <div className="flex items-center gap-2.5 rounded-md p-1.5 hover:bg-muted transition-colors cursor-pointer w-full group">
          <svg width="20" height="20" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-[#22c55e] flex-shrink-0">
          ...
          <span className="text-sm font-semibold tracking-tight text-foreground flex-1 truncate">SBOM.io Workspace</span>
      ...
      <div className="px-3 mb-3 space-y-1">
        <Link
          ...
          onClick={onClose}
          className="flex items-center gap-2.5 px-2 py-1.5 text-[11px] font-medium text-zinc-500 hover:text-[#22c55e] hover:bg-[#22c55e]/5 rounded-md cursor-pointer transition-all"
        >
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-3 mb-2">
          <p className="px-2 mb-1.5 text-[9px] font-bold uppercase tracking-[0.1em] text-[#4a5068]">Main</p>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={onClose}
                  className={cn(
                    'group relative flex items-center gap-2.5 rounded-md px-2 py-1.5 text-[12px] font-medium transition-all duration-150',
                    isActive
                      ? 'bg-[#22c55e]/[0.08] text-foreground'
                      : 'hover:bg-muted/50 hover:text-foreground text-zinc-500'
                  )}
                >
                  {/* Green left-border indicator for active item */}
                  {isActive && (
                    <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[2.5px] h-3.5 rounded-full bg-[#22c55e] shadow-[0_0_8px_rgba(34,197,94,0.5)]" />
                  )}
                  <Icon className={cn(
                    'h-3 w-3 transition-colors ml-1',
        ...
        <div className="px-3 mt-4">
          <p className="px-2 mb-1.5 text-[9px] font-bold uppercase tracking-[0.1em] text-[#4a5068]">Configuration</p>
          <nav className="space-y-0.5">
            <Link
            ...
            >
              {pathname === '/dashboard/settings' && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[2.5px] h-3.5 rounded-full bg-[#22c55e]" />
              )}
              <Settings className="h-3 w-3 text-zinc-500 group-hover:text-zinc-300 ml-1" />
      ...
      <div className="p-3 mt-auto border-t border-border">
        <DropdownMenu>
          <DropdownMenuTrigger className="flex w-full items-center gap-2.5 rounded-md p-1 hover:bg-muted transition-all outline-none group">
            <Avatar className="h-6 w-6 border border-border shrink-0">
              <AvatarImage src={user?.user_metadata?.avatar_url} />
              <AvatarFallback className="bg-[#22c55e]/10 text-[#22c55e] text-[9px]">
            ...
            <div className="flex flex-col items-start overflow-hidden flex-1">
              ...
              <span className="text-[8px] text-zinc-600 truncate w-full">Personal Workspace</span>
            </div>
            <div className="h-1 w-1 rounded-full bg-[#22c55e]/50 mr-0.5 shrink-0" />
          </DropdownMenuTrigger>
          <DropdownMenuContent className="w-64 bg-card border-border text-zinc-400 shadow-2xl p-2" align="start" side="top" sideOffset={12}>
