...
export function SecuritySection({ user, loading }: { user: User | null; loading: boolean }) {
  ...
  const handleDeleteAccount = async () => {
    ...
    setDeleting(true)
    try {
      // Call the Go API endpoint which handles full cleanup before deleting auth user
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-2">
          {providers.map(p => (
            <div key={p} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-border">
              ...
              <Badge className="bg-[#4ade80]/10 text-[#4ade80] border-[#4ade80]/20 text-[10px] font-bold">Connected</Badge>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-muted/20 p-6 space-y-4">
        ...
        <div className="flex items-center justify-between p-4 rounded-lg bg-muted/30 border border-border">
          ...
          <Badge className="bg-[#4ade80]/10 text-[#4ade80] border-[#4ade80]/20 text-[10px]">Active</Badge>
        </div>
        <Button
          variant="outline"
          onClick={handleSignOutAll}
          className="border-border text-xs font-bold h-9 text-muted-foreground hover:text-red-400 hover:border-red-500/30 transition-colors"
        >
      ...
      <div className="rounded-xl border border-red-500/20 bg-red-500/[0.02] p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-0.5 max-w-sm">Permanently delete your account and all associated DEPTIC data. This cannot be undone.</p>
          </div>
          <Button
            onClick={() => setDeleteModal(true)}
            className="bg-red-500/10 border border-red-500/20 text-red-500 hover:bg-red-500 hover:text-foreground transition-all font-bold text-xs h-9 shrink-0"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 justify-end pt-1">
            ...
            <Button
              ...
              className="bg-red-500 hover:bg-red-600 text-foreground font-bold text-xs h-9"
            >
