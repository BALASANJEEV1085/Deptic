...
export async function GET(_: NextRequest, { params }: { params: { scanId: string } }) {
  try {
    ...
    const res = await fetch(`${API_URL}/scans/${params.scanId}/vulnerabilities`, { headers, cache: 'no-store' });
    const data = await res.json();
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch vulnerabilities' }, { status: res.status });
